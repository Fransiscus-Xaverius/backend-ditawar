    ...
    try {
      ...
      if (result) {
        if (result.starting_price > bid) {
          return res
            .status(HTTP_STATUS_CODES.BAD_REQUEST)
            .json({ msg: "Bid is lower than starting price" });
        }
        if (result.highest_bidder == 0 || result.highest_bidder == undefined) {
          ...
          const update = await client
            ...
            .updateOne(
              ...
              {
                $set: {
                  highest_bid: resultt.insertedId,
                  highest_bidder: user._id,
                },
              }
            );
          const newHistory = {
          ...
          const newSaldoTertahan =
            parseInt(wallet!.saldo_tertahan) + parseInt(bid);
          const updateHistory = await client
            .db("dbDitawar")
            .collection("wallets")
            .updateOne(
          ...
          const updateSaldo = await client
            ...
            .updateOne(
              { id_user: new ObjectId(user._id) },
              { $set: { saldo: newSaldo, saldo_tertahan: newSaldoTertahan } }
            );
          logger.log("----------");
          logger.log(`user:, ${user._id}`);
          logger.log(`new saldo: ${newSaldo}`);
          logger.log(`saldo tertahan: ${newSaldoTertahan}`);
          logger.log(`saldo update: ${updateSaldo}`);
          // console.log("----------")
          // console.log("user:", user._id)
          // console.log("new saldo:", newSaldo);
          // console.log("saldo tertahan:", newSaldoTertahan);
          // console.log("saldo update:", updateSaldo);
          const updateBidCount = await client
            .db("dbDitawar")
            .collection("auctions")
            .updateOne({ _id: o_id }, { $inc: { bid_count: 1 } });
          return res
            .status(HTTP_STATUS_CODES.CREATED)
            .json({ msg: "Bid added", result: resultt });
        } else {
          // console.log(result.highest_bid)
          const highestBid = await client
            .db("dbDitawar")
            .collection("bids")
            .findOne({ _id: new ObjectId(result.highest_bid) });
          // console.log("highest:", highestBid);
          if (highestBid && parseInt(highestBid.bid) < parseInt(bid)) {
            logger.log("NEW HIGHEST");
            ...
            const updatePrevBid = await client
              .db("dbDitawar")
              .collection("bids")
              .updateOne(
          ...
          logger.log(`bid: ${bid}, highestBid: ${highestBid?.bid}`);
          //   console.log(bid, highestBid?.bid);
        }
        logger.log("bid is lower");
        // console.log("bid is lower");
        return res
          .status(HTTP_STATUS_CODES.BAD_REQUEST)
          .json({ msg: "Bid is lower than current highest bid" });
      }
    } catch (error) {
      logger.log("error in db insert/update");
      //   console.log("error in db insert/update");
      logger.log(error);
      // console.log(error);
      return res
        .status(HTTP_STATUS_CODES.INTERNAL_SERVER_ERROR)
        .json({ msg: "Internal DB server error" });
    }
  } catch (error) {
    logger.log("error in initial req");
  ...
  try {
    ...
    try {
      ...
      if (result) {
        ...
        if (result.highest_bidder == 0 || result.highest_bidder == undefined) {
          const resultt = await client
            ...
            .insertOne({
              id_auction: o_id,
              id_user: user._id,
              bid: parseInt(result.asking_price),
              highest: true,
              returned: false,
            });
          // console.log(resultt.insertedId);
          ...
          const newSaldoTertahan =
            parseInt(wallet!.saldo_tertahan) + parseInt(result.asking_price);
          const updateHistory = await client
            .db("dbDitawar")
            .collection("wallets")
            .updateOne(
          ...
          return res
            .status(HTTP_STATUS_CODES.CREATED)
            .json({ msg: "Auction Ended", result: resultt });
        } else {
          // console.log(result.highest_bid)
          const highestBid = await client
            .db("dbDitawar")
          ...
          if (
            highestBid &&
            parseInt(highestBid.bid) < parseInt(result.asking_price)
          ) {
            logger.log("NEW HIGHEST");
            // console.log("NEW HIGHEST");
            const resultt = await client
            ...
            try {
              ...
              const result = await client
                ...
                .updateOne(
                  { _id: o_id },
                  { $set: { tanggal_selesai: new Date(), ended: true } }
                );
            } catch (error) {
              logger.log(error);
              // console.log(error);
          ...
          logger.log(
            `result.asking_price: ${result.asking_price}, highestBid?.bid: ${highestBid?.bid}`
          );
        ...
        return res
          .status(HTTP_STATUS_CODES.BAD_REQUEST)
          .json({ msg: "Asking Price is lower than current highest bid" });
      }
    } catch (error) {
      logger.log("error in db insert/update");
      logger.log(error);
...
async function HighBid(req: Request, res: Response) {
  ...
  try {
    ...
    const result = await client
      .db("dbDitawar")
      .collection("bids")
      .findOne({ id_auction: o_id, highest: true });
    return res
      .status(HTTP_STATUS_CODES.CREATED)
      .json({ msg: "Item Found", result: result });
  } catch (error) {
...
async function getAllBid(req: Request, res: Response) {
  const { id_auction } = req.query;
  try {
    await client.connect();
...
async function BidUpdate() {
  try {
    ...
    const result = await client
      .db("dbDitawar")
      .collection("bids")
      .find({ $and: [{ highest: false }, { returned: false }] })
      .toArray();
    for (let i = 0; i < result.length; i++) {
      ...
      const wallet = await client
        .db("dbDitawar")
        .collection("wallets")
        .findOne({ id_user: new ObjectId(bid.id_user) });
      const saldo = wallet!.saldo;
      ...
      const update = await client
        ...
        .collection("wallets")
        .updateOne(
          { id_user: new ObjectId(bid.id_user) },
          { $set: { saldo: newSaldo, saldo_tertahan: newSaldoTertahan } }
        );
      const newHistory = {
        wallet_id: wallet!._id,
...
export {
  ...
  getBid as getBid,
  buyNow as buyNow,
  BidUpdate as BidUpdate,
  HighBid as HighBid,
  getAllBid as getAllBid,
};
